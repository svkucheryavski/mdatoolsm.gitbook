...
The general syntax is following. First one creates and empty preprocessing object. Then add or remove methods by using methods `add('name', param1, param2, ...)` and `remove(n)`. Below shown a table with all currently available methods and their parameters.
