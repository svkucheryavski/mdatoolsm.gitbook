...
In fact, `ypred` is not actually a property, but a method that gives an access to the predicted values. The method always return values as an `mdadata` object, so we can use e.g. `show()` to see the values. By default it returns predicted values for the first y-variable and all components.
