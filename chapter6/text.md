...
All plots available for PLS results are also available for PLS model. Similar to PCA and MLR plots for the model are group plots, showing values for each result available (calibration, cross-validation and test-set validation) and discriminate the results using colors. 
