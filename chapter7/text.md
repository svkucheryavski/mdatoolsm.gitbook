...
It must be noted that PLS-DA in general supports multiclass classification, when one provides the binary values in several columns (one for each class). However, it is not recommended to do this (see e.g. explanation here) and better create several one-class PLS-DA models instead of one for multiple classes. 
