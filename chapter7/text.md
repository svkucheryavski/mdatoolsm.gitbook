...
It must be noted that PLS-DA in general supports multiclass classification, when one provides the binary values in several columns (one for each class). However, it is not recommended to do this (see e.g. an explanation [here](http://onlinelibrary.wiley.com/doi/10.1002/cem.2609/abstract)). In this case it is better create several one-class PLS-DA models instead of one for multiple classes. 
